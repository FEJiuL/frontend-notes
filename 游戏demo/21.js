...
class Player {
  ...
  getCard(card) {
    //将卡牌A放到第一位，计算点数时最后计算
    if (card.val !== "A") this.cards.push(card);
   ...
   */
  demandCard() {
    if (this.demand && this.count > 0 && this.count < 18) return this;
    return false;
  }
  /**
   * ==== 计算点数 ====
  ...
  calculate() {
    this.count = this.cards.reduceRight((total, c) => {
      //如果牌面值为A，判断总点数是否超过10
      if (c.val === "A" && total <= 10) return total + 11;
  ...
  executeQueue(queue) {
    ...
    setTimeout(() => {
      if (queue.length > 0) {
        let player = queue[0];
        this.dealCard(player);
        if (!player.demandCard()) queue.shift();
